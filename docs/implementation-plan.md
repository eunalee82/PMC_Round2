...
### 8.5 참가자 식별 — 확정 (2026-08-03)

**(A) 팀 단위 진행 + 팀원 이메일 등록 + 팀 점유(claim)** 로 확정. 팀별 비밀번호는 폐지했다(37팀 배포·재배포 비용, 결과 데이터 검수 불가).

- **입장 = 등록**: 팀 선택 후 팀원 **3명 고정**의 **전체 이메일**(도메인 포함)을 입력하면 입장. 첫 입장 기기가 그 팀을 **점유**한다(팀당 1기기).
- **재입장 = 등록 이메일 확인**: 다른 기기가 같은 팀에 접근하면 등록된 3개 중 **하나를 입력해야** 인계된다. 배포할 비밀이 없는 자연 비밀번호.
- **기기 식별**: `localStorage`의 `pmb.device.v1` 세션키. 세션 복구는 이 키로, 기기 교체 복구는 이메일로.
- **입력 정책**: 정규화(공백 제거·소문자·전각 `＠`·끝 문장부호 제거) 후, **빈칸·팀 내 중복만 차단**하고 형식 의심은 경고 후 통과 + `suspect` 플래그. 운영 원칙은 "입력을 막지 않는다".
- **검수**: 같은 이메일이 2개 팀에 등록되면 `cross-team-duplicate`로 표시(팀 오선택 신호). 운영진이 대기 모드에서 정정 후 시작.
- **결과 데이터**: `team_no, team_name, member1~3_email, entered_at, flags` CSV.

구현: `src/js/lib/entries.js`(mock, localStorage) · `src/js/utils/email.js` · `src/components/dev/entry-monitor.js`.
서버 이관 시 `team_entries`(`team_id` UNIQUE) + `claim_team` / `verify_member` / `release_team` RPC로 `entries.js` 내부만 교체한다.
...
2. ~~**참가자 식별/인증**~~ — **확정(2026-08-03)**: 팀 선택 + 팀원 3명 이메일 등록 + 팀 점유, 재입장은 등록 이메일 확인. 팀별 비번 폐지. → §8.5
...
6. **팀 구성 데이터** — 팀당 **3명 고정**은 확정(2026-08-03). 팀 수·이름·컬러는 현재 mock 37팀(`src/js/mocks/teams.js`, DEV "팀 관리"에서 편집) → 운영 확정 명단으로 교체 필요.
