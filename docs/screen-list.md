...
> 기본 흐름은 공개 연습 모드(solo)로 바뀌었고, 거기에는 **SCR-003 팀 선택과 SCR-005 대기실이 없다**
> (Entry → Opening → 서약 → 사건). 행사 모드는 `?event` 로 들어가며 이 문서 그대로 동작한다.
> 두 모드의 차이는 `CLAUDE.md §16.3`, 스위치는 `src/js/lib/mode.js`.
...
| SCR-015 | Officer Appointment | 참가자 | 감독관 임명 |
...
참가자가 자신이 소속된 팀을 선택하고, **팀원 3명의 이메일을 등록**하여 입장한다.
등록이 곧 입장이며, 등록한 기기가 그 팀을 **점유**한다(팀당 1기기). 팀별 비밀번호는 사용하지 않는다.

## 주요 구성 요소

- 팀 선택 카드 (팀명 · 식별 색상 · 참가 인원 · 점유 상태)
- 수사관 등록 모달 (이메일 3칸)
- 등록된 수사관 확인 영역 (이메일 칩 + 수정)
- 사건 접수 버튼
...
| 상태 | 표시 | 클릭 시 |
|---|---|---|
| 미입장 | 자물쇠 · `수사관 3명` | 수사관 등록 모달 |
| 이 기기가 점유 | 체크 · `등록 완료 · HH:MM` | 등록 수정 모달 |
| 다른 기기가 점유 | 회색 · `입장 완료 · HH:MM` | 재입장 모달 |
...
```text
[7팀 · 수사관 등록]
팀원 3명의 이메일 주소를 모두 입력하십시오.
등록된 이메일은 재접속 시 팀 확인에 사용됩니다.

수사관 1  [ name@company.com ]
수사관 2  [ name@company.com ]
수사관 3  [ name@company.com ]

               [취소]  [입장]
```

- 입력값은 blur·붙여넣기 시 정규화된다: 공백 전부 제거 → 전각 `＠` → `@` → 소문자 → 끝 문장부호 제거
- **차단**: 빈칸, 팀 내 중복 이메일 (3명 고정이므로 명백한 오류) → `[입장]` 비활성
- **경고 후 통과**: 형식 의심(`@` 없음, 도메인에 `.` 없음, 한글 포함 등) → 노란 안내 + `suspect` 플래그

## 재입장 모달 (다른 기기가 점유한 팀)

```text
[7팀 · 이미 입장한 팀입니다]
15:42 입장 · eu***@lge.com 외 2명

우리 팀이 맞다면 등록된 수사관 이메일 중 하나를 입력하십시오.
[                        ]

        [다른 팀 선택]  [이어서 입장]
```

등록된 3개 중 하나가 일치하면 점유가 인계되고, 이전 기기는 다음 이동 시 팀 선택으로 되돌아간다.
...
- team_name
- team_color
- member_emails (3개, 정규화된 전체 주소)
- entered_at
- device_id (점유 기기)
- flags (`suspect` / 조회 시점 계산: `cross-team-duplicate`)

## 화면 전환

```text
수사관 3명 등록 완료 → [사건 접수 시작]
→ SCR-004 Oath Agreement
```

## 검증

- 팀 미선택·미등록 시 진행 불가 (SCR-004·005 접근 차단)
- 존재하지 않는 팀 접근 차단
- 팀원 3명 고정 — 3칸 모두 필수
- 이미 점유된 팀은 등록 이메일 확인 없이 입장 불가
- 기기당 1팀 — 다른 팀으로 재등록하면 이전 팀 점유는 해제된다
- 등록 정보는 세션과 서버에 저장, 새로고침·재접속 시 복구
- 운영진이 입장을 해제하면 해당 기기는 안내 후 이 화면으로 되돌아온다 (SCR-104)
...
- 서약문
- 선택한 팀명
- 등록된 수사관 3명 (SCR-003에서 입력한 이메일 — 팀 오선택을 알아차릴 마지막 지점)
- 이름 또는 서명 입력
- 동의 체크박스
- 서약 완료 버튼
...
모든 Stage를 통과한 참가자를 정식 감독관으로 임명한다.
...
감독관 임명 직후 빌런왕 출현을 알리고 Final Raid로 전환한다.
...
- 입장 현황 보드 (`입장 N / 전체`, 전체·입장·미입장·확인 필요 필터)
- **입장 해제** — 오선택한 팀을 게임 시작 전에 되돌린다 (해제 시 해당 기기는 팀 선택으로 복귀)
- CSV 내보내기 (`team_no, team_name, member1~3_email, entered_at, flags`)
- 팀 상세 보기
- 진행 단계 복구
- 연결 오류 확인
- 점수 재계산
...
| 플래그 | 조건 | 의미 |
|---|---|---|
| 형식 의심 | 등록 시 `suspect` | 이메일 형식이 어긋남 (입장은 허용됨) |
| 타 팀 중복 | 같은 이메일이 2개 이상 팀에 등록 | 둘 중 한 팀이 팀 선택을 잘못했다 |

> 게임 시작 전 대기 모드에서 이 보드를 확인하고 정정 요청 후 Start 한다.
> DEV 프로토타입: `src/components/dev/entry-monitor.js` (DEV 메뉴 → 입장 현황)
...
> "팀 선택" 열은 **팀 선택 + 수사관 3명 등록(입장)까지 완료**된 상태를 뜻한다(SCR-003).
> 팀만 고르고 이메일을 등록하지 않은 상태는 "미등록"으로 취급해 Oath 이후를 차단한다.
