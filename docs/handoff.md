# 세션 핸드오프 (최신: 2026-08-11)

세션별 작업 기록. **저장소를 처음 여는 사람은 `README.md` → `CLAUDE.md` 를 먼저 읽는다.**

> ## ⚠️ 이 문서는 시간순 기록이다 — 아래 §0~§9 는 **과거 상태**다
>
> 앞쪽 절에는 이후 뒤집힌 내용이 그대로 남아 있다(예: "Supabase 미연결(MOCK)", "Stage 2 는 임시 콘텐츠",
> "관리자 비번 2026"). **당시 판단의 근거를 남기려고 지우지 않았을 뿐이며, 현재 상태가 아니다.**
> 최신 상태는 **§11 · §12 와 아래 요약**을 본다.
>
> ### 현재 상태 (2026-08-11 마감)
>
> - **프로덕션 배포 완료** `https://pmc-round2.vercel.app` · 커밋·push 완료
> - **마이그레이션 `0001`~`0014` 전부 적용** (`node scripts/check-migrations.mjs` 로 재확인)
> - **Supabase 연결됨** — 진행·점수·정답 판정은 전부 서버 RPC. mock 은 비상 경로로만 남아 있다
> - **사건 15개 전부 확정** — 임시(placeholder) 데이터 없음. 본문·정답은 **서버에만** 내려간다(0012·0013)
> - **관리자 콘솔은 Supabase Auth 로그인** — 비번 `2026` 은 mock 모드 전용이다
> - 참가자 계급 용어: **신입 감독관 → 정식 감독관** (2026-08-11 변경, `CLAUDE.md §15`)
> - 32팀 동시 진행 부하 시뮬레이션 통과(§12.12) · 게임 상태는 `scheduled` 로 초기화해 둠
> - 유일한 미해결 버그: **Final Raid BGM 미재생**(§5)
>
> ### 리허설에서 한 번 확인할 것
>
> - 관리자 콘솔 **[+5분 연장]** 실제 동작 (RPC 존재·anon 차단까지는 확인, 관리자 클릭은 미확인)
> - 관리자 초기화 시 참가자 기기가 로컬을 지우고 첫 화면으로 되돌아가는지 (`flow.js:169-173`)

---

## 0. 지금 당장 알아야 할 것 (TL;DR) — ⚠️ 2026-08-06 시점 기록
